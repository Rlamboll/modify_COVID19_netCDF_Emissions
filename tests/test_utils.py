...
temp[:, :, :, :] = (
    times[:].reshape(timeslen, 1, 1, 1)
    * sectors[:].reshape(1, sectlen, 1, 1)
    * lats[:].reshape(1, 1, latlen, 1)
    * lons[:].reshape(1, 1, 1, lonlen)
)
...
def test_insert_interpolated_point():
    ...
    assert all(
        new_scc.variables["temp"][:, :, :, :].shape[i] == startshape[i]
        for i in range(1, 3)
    )
    np.allclose(new_scc.variables["time_bands"][:, 0:5], startband[:, 0:5])
    np.allclose(new_scc.variables["time_bands"][:, 7:], startband[:, 6:])
    np.allclose(
        new_scc.variables["time_bands"][:, 6], (startband[:, 5] + startband[:, 6]) / 2
    )
    assert new_scc.variables["time"][6] == newtime
    assert np.allclose(new_scc.variables["temp"][6, :, :, :], expected_results)
    assert np.allclose(
...
def test_cleanup_files():
    ...
    new_scc.close()
    new_scc = cutoff_netcdf_time(folder, folder, test_file, 1, scenario_string=work_str)
    new_scc.close()
